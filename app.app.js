...
class Finanzas {
  ...
  load(){
    ...
    const seed={}; seed[this.iniMes]={
      ingresos:[{id:this.uid(),nombre:"Salario",monto:3500000,fecha:"01"}],
      gastosFijos:[{id:this.uid(),nombre:"Arriendo",monto:1200000,fecha:"01",paid:false}],
      tarjetas:[],
      creditos:[],
        ...
        if(Array.isArray(copy[k])){
          copy[k]=copy[k].map(item=>{
            ...
            if(k === 'tarjetas' || k === 'creditos') {
              ...
              // Incrementar cuotas pagadas automáticamente al crear nuevo mes
              if(it.cuotasPagadas < it.numeroCuotas) {
  ...
  duplicateToNextMonth(){
    ...
    this.toast(`Mes duplicado a ${meses[parseInt(toKey)]} - Cuotas actualizadas automáticamente`);
  }
  ...
  rowGeneric(icon,i,key,monto){
    ...
    return `<div class="item ${paidClass}">
      <div class="row">
        <div>${icon} <b>${i.nombre}</b><div class="meta">${i.fecha||""}</div></div>
  ...
  openForm(tipo,item=null){
    ...
    this.showModal(title, fields, (vals)=>{
      ...
      if(tipo==="ingreso"){
        ...
        d.gastosCompras.push({id:this.uid(),nombre:vals.nombre,monto:n(vals.monto),fecha:vals.fecha,paid:false});
      }else if(tipo==="ahorro"){
  ...
  edit(key,id){
    ...
    this.showModal(title, fields, (vals)=>{
      ...
      if(!isDeuda && key!=="ahorros"){
        Object.assign(it,{nombre:vals.nombre,monto:n(vals.monto),fecha:vals.fecha});
      }else if(key==="ahorros"){
